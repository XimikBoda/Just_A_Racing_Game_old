...
public class CarControl : MonoBehaviour
{
    ...
    {
        float RL_cof = (float)Math.Pow(maxrpm, -Math.Abs(RLCollider.rpm));
        float RR_cof = (float)Math.Pow(maxrpm, -Math.Abs(RRCollider.rpm));
        ...
        else
        {
            ...
            float RLCof2 = horizontalInput > 0 ? radiusOutside / radiusInside : radiusInside / radiusOutside; //Coeffitient for turning left/right
        ...
        Debug.Log(RLCollider.rpm + " " + RRCollider.rpm); 
    ...
    {
        float steerCoef = 1 - (float)Math.Log10((gameObject.GetComponent<Rigidbody>().velocity.magnitude / 40f) + 1f);
